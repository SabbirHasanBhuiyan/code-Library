// Original Author: Shahjalal Shohag
...
struct SuffixArray {//0 means 1st..
  int n;//rank[i]=position suffix starting at index in SA
  string s;//sa[i]=starting index of the i-th smallest suffix
  ...
  int get_lcp(int i,int j){//lcp of 2 suf from s[i] & s[j]
    if (i == j) return n - i;
    int l = rank[i], r = rank[j];
    if (l > r) swap(l, r);
    return query(l, r - 1);
  }
  int lower_bound(string &t) {//finds first suffix >= t
...
int32_t main() {
    ...
    cin >> s;
    SuffixArray sa(s);
    for(int i=0;i<sa.sa.size();i++) cout<<sa.sa[i]<<'\n';
